...
export class AsignacionesIndexComponent implements OnInit {
  ...
  BorrarAsignacion(eventMessage: string) {
    ...
    console.log("asignaciones:"); console.log(this.asignaciones);
    ...
    this.asignacionesService.GetAsignacionesPaginacion(noPagina).subscribe(data => {
      ...
      this.proyectosService.GetProyectos().subscribe(data => {
        ...
        for (let i: number = 0; i < this.asignaciones.length; i++) {
          proyecto = proyectos.find(project => project.id_proyecto === this.asignaciones[i].id_proyecto);
      ...
      this.empleadosService.GetEmpleados().subscribe(data => {
        ...
        for (let i: number = 0; i < this.asignaciones.length; i++) {
          empleado = empleados.find(employee => employee.id_empleado === this.asignaciones[i].id_empleado);
          this.empleadosNames[i] = empleado.id_empleado + ".- " + empleado.nombre + " " + empleado.apellido;
        }
      });
    });
  }
