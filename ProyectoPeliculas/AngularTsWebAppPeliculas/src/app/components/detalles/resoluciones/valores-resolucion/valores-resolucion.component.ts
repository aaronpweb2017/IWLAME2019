    ...
    this.detallesTecnicosService.getValoresResolucion().subscribe(
      response => {
        if (response[0]) {
          ...
          this.currentItemsPerPage = this.valoresResolucion.slice(this.paginationConfig.itemsPerPage
    ...
    this.detallesTecnicosService.crearValorResolucion(this.nuevoValorResolucion).subscribe(
      response => {
        if (response[0]) {
          ...
          this.router.navigate(['/adminDetalles', { currentPageValoresResolucion: this.currentPage }]); return;
        }
        this.toastrService.error(response[1]);
      }, error => {
        this.toastrService.error(error.message);
